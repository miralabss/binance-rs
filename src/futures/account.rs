...
impl FuturesAccount {
    ...
    // Custom order for for professional traders
    pub fn custom_order_fast(&self, order_request: &str) -> Result<String> {
        self.client
            .post_signed_fast(API::Futures(Futures::Order), order_request.to_string())
    }
    ...
    pub fn modify_order(&self, order_request: CustomOrderRequest) -> Result<Transaction> {
        ...
        let request = build_signed_request(order, self.recv_window)?;
        self.client
            .put_signed(API::Futures(Futures::Order), request)
    }
